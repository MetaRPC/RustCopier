...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let master = demo.open_demo_account("MetaQuotes-Demo", api_key).await?;
    ...
    let conn = demo.connect_ex(master.login, &master.password, &master.server, api_key).await?;
    ...
    let disc = demo.disconnect(&conn.terminal_instance_guid, api_key).await?;
